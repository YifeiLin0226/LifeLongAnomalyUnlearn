...
class LogDataProcessor:
    ...
    def _session_window(self):
        ...
        for blkId, group in tqdm(grouped):
            ...
            new_data.append({
                'BlockId': blkId,
                'Timestamp': group['Timestamp'].values.tolist(),
                'DeltaT': dt.tolist(),
    ...
    def _generate_train_test(self, data, n_train = None):
        """
        ...
        print(f'train size: {len(train)}, test_normal size: {len(test_normal)}, test_abnormal size: {len(test_abnormal)}')
        ...
        print(f'train logs: {num_train_logs}, test_normal logs: {num_test_normal_logs}, test_abnormal logs: {num_test_abnormal_logs}')
        
        return train, test_normal, test_abnormal
    ...
    def process(self, n_train = None, window_size = None, min_seq = None, seq_len = None, adaptive_window = False):
        ...
        if self.dataset != "OpenStack":
            train, test_normal, test_abnormal = self._generate_train_test(new_data, n_train = n_train)
        else:
